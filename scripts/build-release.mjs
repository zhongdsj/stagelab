/**
 * 单包发布构建：将 monorepo 打包为单一 npm CLI 包 fourstage。
 *
 * 产物布局（发布包 files 只含 dist）：
 * - dist/cli.js   server + shared 业务代码经 esbuild bundle（运行时第三方依赖外部化）
 * - dist/web/     web 生产构建产物（由 fourstage start 静态托管）
 *
 * 运行时依赖声明在根 package.json dependencies 中，安装 fourstage 时由 npm 解析。
 */
...
run("npm run build -w @fourstage/shared");

console.log("[build:release] 构建 web ...");
run("npm run build -w @fourstage/web");
...
await build({
  ...
  format: "esm",
  target: "node18",
  // 第三方运行时依赖外部化，由根 package.json dependencies 安装；@fourstage/shared 被 bundle 进业务代码
